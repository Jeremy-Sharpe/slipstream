...
|---|---|---|
| Anna | `fixtures`, `scorecard`, `icp`, `leads`, `outreach`, `live-extraction`, `model-bakeoff` | Fixtures, scorecard, ICP and live extraction are merged. Her work now feeds the OpenRouter-only demo bootstrap; the optional real Origami adapter remains untested with a credential. The Eleno seller rebrand is in progress on `feat/eleno-fixtures`: rewritten call fixtures, public-client CRM rows, retuned draft safety vocabulary, a regenerated UI fixture mirror and the docs that quote any of it. |
