...
| Thing | Where | State |
|---|---|---|
| Production UI | https://slipstream-app-seven.vercel.app | The judged URL (Anna, 14 Sep 11:44 AEST). A Vercel project outside Anna's teams and outside the repo's GitHub integration, so it only updates when its owner deploys main by hand. At 11:42 AEST it still ran PR 89 without the uncapped won-deals list from `4deca88`, so Intelligence did not yet show the seven Eleno clients; the owner needs to redeploy main. |
| Backup UI | https://slipstream-hackathon.vercel.app and https://slipstream.3-104-149-193.sslip.io | hackathon is Jeremy's Vercel project wired to the repo: it rebuilds on every push to main and at 11:41 AEST showed all twelve won deals including the seven Eleno clients. The VPS runs `fdac2d1` (pre-PR 89) as a standalone Next web behind Caddy, deployed by Anna over Tailscale (`agent-vps`) with `SLIPSTREAM_INGEST_TOKEN` in a root-owned web env file. The API also allows `slipstream-live.vercel.app` and `slipstream-app-kappa.vercel.app` as origins. |
