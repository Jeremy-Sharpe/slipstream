...
| Thing | Where | State |
|---|---|---|
| Production UI | https://slipstream-app-seven.vercel.app | The judged URL, decided 14 Sep 11:26 AEST. Vercel build of main at `b36e30e` (PR 89: landing page at the root, coach entry hidden) wired through the same-origin gateway to the production API below. Verified 14 Sep 11:19 to 11:25 AEST: `/`, `/home`, `/conversations`, `/leads`, `/intelligence` and `/loop` return 200, the gateway proxies `/ready`, ICP, leads and demo evidence, and Intelligence shows the version 7 ICP as Current. |
| Backup UI | https://slipstream.3-104-149-193.sslip.io and https://slipstream-hackathon.vercel.app | Both run the main build before PR 89 (coach entry still in the sidebar). The VPS is a standalone Next web behind Caddy, deployed by Anna over Tailscale (`agent-vps`) with `SLIPSTREAM_INGEST_TOKEN` in a root-owned web env file, moved to `fdac2d1` on 14 Sep 09:59 AEST. The API also allows `slipstream-live.vercel.app` and `slipstream-app-kappa.vercel.app` as origins. |
