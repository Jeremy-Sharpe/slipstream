...
| Thing | Where | State |
|---|---|---|
| Production UI | https://slipstream-hackathon.vercel.app | Vercel deployment `dpl_F17QVRsNEv2zMTVq7NXLyzq77xJV` serves the PR #53 Revenue DNA card, OpenRouter-first sourcing copy, dynamic Revenue Loop and the exact paused campaign in first-response HTML. PRs #56-57 add value cards, mobile-shell polish and the outcome shock test; they are locally browser-verified but not yet promoted because Vercel's `api-deployments-free-per-day` quota rejected both the GitHub hook and one authenticated CLI attempt. Do not retry until the one-day provider window resets. |
