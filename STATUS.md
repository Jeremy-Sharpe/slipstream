...
3. Restart the API once: a stray `POST /icp/derive` on 13 Sep created ICP v2 in memory, and ten leads from v1 still report `leads_needing_rescore: 10` in `/demo/evidence`. The current profile has ten fresh leads.
