...
| Thing | Where | State |
|---|---|---|
| Production UI | https://slipstream.3-104-149-193.sslip.io | Standalone Next web live behind Caddy, deployed by Anna over Tailscale (`agent-vps`) with `SLIPSTREAM_INGEST_TOKEN` in a root-owned web env file, so scoring, playbook, pasted emails and transcription are unlocked. The new UI on the real API (signed-in sample user Liam Albrecht) has been live since 13 Sep 20:02 AEST at revision `8111ea6`; both web and API were moved to `fdac2d1` on 14 Sep 09:59 AEST, a release whose only diff from `8dad6d9` is eval tooling and docs. All six screens and the run page were verified in a browser on the live URL, and `/`, `/loop`, `/conversations`, `/intelligence` and `/leads` all return 200 after the move. |
| Backup UI | https://slipstream-hackathon.vercel.app | Redeployed with the current UI on 13 Sep evening: it now answers the `/gateway` routes the new UI needs (checked 14 Sep 11:05 AEST). It is not Anna's Vercel project (hers has no deploy newer than 12 Sep and no ingest token). The API also allows `slipstream-live.vercel.app` and `slipstream-app-kappa.vercel.app` (both serve the gateway) and `slipstream-ten-mauve.vercel.app` (old build, no gateway) as origins. The judged URL stays the VPS. |
...
2. Clear the 30 stale leads (ICP versions 1 to 4) from Supabase or rescore them against version 5, so `/demo/evidence` stops reporting `leads_needing_rescore: 30` and the production smoke passes again. Every `POST /api/v1/demo/bootstrap` that derives a new profile version leaves the previous version's leads behind in durable storage, so do not bootstrap again until this is handled.
