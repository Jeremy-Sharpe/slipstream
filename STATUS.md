...
| Thing | Where | State |
|---|---|---|
| Production UI | https://slipstream.3-104-149-193.sslip.io | Standalone Next web revision `a21e2ba` is live behind Caddy and includes the value cards, outcome shock, complete five-route phone-width polish and the live Campaigns Revenue DNA spend gate. Full production contract smoke passes; Chromium proves the outcome shock, current campaign fingerprint, evidence deep-link and zero overflow at 1440px and 390px. The service is active with zero crash restarts. |
...
| Production API | https://slipstream-api.3-104-149-193.sslip.io | Live at exact revision `16520b8dc829679b0e5461cedd7a2be77d367d6f`; readiness verifies OpenRouter `openai/gpt-5.4` reasoning and `text-embedding-3-small`. The bounded demo evidence endpoint verifies the pre-rebrand cohort of 13 deals, 12 calls, two emails, ten safe fictional prospects and a current Revenue DNA fingerprint; the Eleno fixtures raise the eligible cohort to 19 deals and 12 calls once this branch is redeployed. Storage is `memory`; Supabase, Origami and delivery remain disabled. |
...
3. Deploy the `feat/max-ui` web build to the VPS and add `Environment=SLIPSTREAM_INGEST_TOKEN=<the API INGEST_TOKEN>` to `deploy/web/slipstream-web.service`; without it scoring, playbook, pasted emails and transcription show a locked message on production.
4. Restart the API once: a stray `POST /icp/derive` on 13 Sep created ICP v2 in memory, and ten leads from v1 still report `leads_needing_rescore: 10` in `/demo/evidence`. The current profile has ten fresh leads.
5. Re-record the demo video on the new UI; the linked v2 video shows the earlier screens.
