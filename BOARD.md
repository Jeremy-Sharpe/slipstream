...
| `web-wire-analysis` | Replace mock analysis and ICP data with live reads | `app/analysis` | Jeremy's agent | feat/web-wire-analysis | in progress | Wire the Intelligence surface to API-derived scorecards/ICP where configured, with explicit labelled-eval fallback when model credentials are absent |
