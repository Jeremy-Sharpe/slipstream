...
| `email-ingest` | Ingest inbound/outbound email threads, map participants into CRM-shaped records, and draft a grounded reply from thread history | `api/app/routers/emails.py`, `api/app/services/email.py` | Jeremy's agent | feat/email-ingest | in progress | 84 tests pass and review blocker fixed; tenancy, atomic Supabase ingestion, recipient roles, and draft versioning remain before merge |
