...
| `web-wire-leads` | Replace mock leads with live Origami results, similarity score, outreach approve | `app/leads` | Jeremy's agent | feat/web-wire-leads | in progress | Connect the lead table, sourcing state and outreach approval to the deployed API with explicit simulated-data fallback while Origami credentials are absent |
