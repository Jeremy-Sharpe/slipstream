...
| `scorecard` | Rubric document, LLM-as-judge scorecard per call, ten-call labelled eval and script | `api/app/services/score.py`, `api/evals/` | Anna | feat/scorecard | done | Rubric v1, evidence-validated judge, playbook step, `POST /scorecards` and `POST /playbook`; eleven-model bake-off run, DeepSeek V3.2 picked (see `api/evals/README.md`); live on the VPS once main redeploys |
