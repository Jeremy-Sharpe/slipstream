...
| `web-wire-conversations` | Replace mock conversations with Supabase reads and API calls: transcript, extracted fields with approval, scorecard, draft | `app/` conversation views, `lib/` | Jeremy's agent | feat/web-wire-conversations | in progress | Integrating Max's `feat/ui-base` without rewriting its Clay-style information architecture; use the live API with an explicit demo fallback |
