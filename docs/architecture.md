...
- **Scorecards are stored on the conversation row.** A stored call is scored by id through `POST /calls/{conversation_id}/scorecard`; the transcript is rebuilt from the stored segments, the rep and outcome come from the fixture script for fixture calls or from the request body and the stored extraction otherwise, and the result is written to `conversations.scorecard` (memory store when Supabase is not configured) with the model and rubric version that produced it. Repeat calls return the stored scorecard unless `force=true`.

## The learning loop (playbook)

Anna's design (12 September): score every call, then learn from the won ones. `derive_playbook` computes won-versus-not-won statistics per rubric dimension and a rolling profile per rep, then asks the model to name three to five behaviours that separate the won calls, each citing call ids and verbatim quotes. A pattern is dropped if it cites a call that is not in the input or a quote that does not appear in that call's evidence or summary. The output is the analysis surface's "what wins" lens and the seed for coach prompts. No-show calls are excluded from the means. `POST /playbook/derive` runs it over every stored scorecard and the result is cached in the API process behind `GET /playbook/latest`; persisting playbooks to a table is a follow-up.
...
- **12 Sep 2026, Anna**: scorecard wired to stored calls (`POST /calls/{id}/scorecard`, `GET /calls/{id}/scorecard`, `GET /scorecards`) writing `conversations.scorecard`; playbook derived from stored scorecards (`POST /playbook/derive`, `GET /playbook/latest`).
