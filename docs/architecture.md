...
- **Coach provider coverage.** The coach tests are now isolated from developer-shell provider credentials and all 174 API tests pass. A future credentialled staging check should still exercise live token minting and WebSocket recovery against the selected providers.
