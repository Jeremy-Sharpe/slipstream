...
Production URL: https://slipstream-hackathon.vercel.app (current UI, deployed from main by Vercel, wired to the production API below)

Backup UI: https://slipstream.3-104-149-193.sslip.io (VPS, older build) and https://slipstream-app-seven.vercel.app (older build)
...
| Live coach suggestions and card completion | The same `REASONING_MODEL` as extraction (now `mistralai/mistral-medium-3.1`), prompt `coach-session-v2.md`, plus deterministic guards; the spot check below was run on `openai/gpt-5.4`, and on Mistral the analysis JSON came back truncated on two runs of the same ten cases, so the coach is not part of the demo path until it has its own model setting or is removed | Deciding a question was asked or answered hides a card, so precision comes first. A ten-case spot check (`api/evals/coach-eval.md`) went from 0.60 to 1.00 precision at 1.00 recall across three runs as the prompt and guards were fixed; median analysis 2.8 s. Ten cases and one run each, so this is a spot check, not a benchmark |
...
- Five fixture calls ship with playable recordings on Home's call picker and the call page. The demo call is voiced with ElevenLabs; the won, stalled, lost and no-show examples were voiced with the open-weight Kokoro-82M model because the ElevenLabs key was not on the machine that generated them (`fixtures/publish_recordings.py`). Running one of these uses the fixture's script as the transcript, not a fresh Scribe pass over the audio.
...
- The production extractor, Mistral Medium 3.1, misread the annual deal value on five of the thirteen rebranded fixture calls (monthly or per-seat figures reported as the contract value); gpt-5.4 gets all thirteen but labels objection handling on one call in thirteen, and neither clears the bake-off's one-call rule on this cohort. The trade is recorded in `api/evals/extraction-eval.md`.
