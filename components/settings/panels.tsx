    ...
    <Card title="Workspace" description="What the team sees at the top of every page and how numbers are shown.">
      <form className="grid max-w-xl grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); save(); }}>
        ...
        <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Timezone</span>
          <select value={ws.timezone} onChange={(e) => setWs({ ...ws, timezone: e.target.value })} className="h-9 rounded-md border border-border bg-card px-3 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">
        ...
        <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Currency</span>
          <select value={ws.currency} onChange={(e) => setWs({ ...ws, currency: e.target.value })} className="h-9 rounded-md border border-border bg-card px-3 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">
    ...
    <Card title="CRM" description="Slipstream writes calls, notes, tasks and drafts into your CRM. Nothing changes without an approval.">
      <div className="flex items-start justify-between gap-6 rounded-lg border border-border p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="flex items-center gap-2 text-[15px] font-semibold text-foreground">HubSpot <span className="flex items-center gap-1.5 rounded-md border border-border px-2 py-0.5 text-xs font-normal text-foreground/80"><StatusDot on={connected} />{connected ? "Connected · mock" : "Disconnected"}</span></p>
            {connected ? (
              <p className="mt-1 text-sm text-muted-foreground">Portal {hubspot.portalId} · {syncedJustNow ? "Synced just now" : `Synced ${hubspot.lastSyncMinutesAgo} min ago`} · {hubspot.objects.join(", ")}</p>
            ) : (
              <p className="mt-1 text-sm text-muted-foreground">Connect a portal to write calls and drafts back to your records.</p>
            )}
          </div>
        </div>
      ...
      <p className="mt-3 text-xs text-muted-foreground">Object mapping: contacts, companies, deals, calls, notes, tasks and emails mirror the HubSpot objects, so a real integration is a field mapping rather than a redesign.</p>
    </Card>
  );
}
    ...
    <Card title="Team" description="Who can see conversations and approve drafts.">
      <div className="overflow-hidden rounded-lg border border-border">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border bg-page text-left text-xs font-medium text-muted-foreground"><th className="px-4 py-2.5">Member</th><th className="px-4 py-2.5">Email</th><th className="px-4 py-2.5">Role</th><th className="px-4 py-2.5">Status</th><th className="w-12" /></tr></thead>
          <tbody>
            {members.map((m) => (
              <tr key={m.id} className="border-b border-border last:border-b-0">
                <td className="px-4 py-2.5"><span className="flex items-center gap-2.5"><span className="flex size-7 items-center justify-center rounded-full bg-avatar text-[11px] font-medium text-foreground">{initials(m.name)}</span><span className="font-medium text-foreground">{m.name}</span></span></td>
                ...
                <td className="px-4 py-2.5"><span className="flex items-center gap-1.5 text-foreground/80"><StatusDot on={m.status === "active"} />{m.status === "active" ? "Active" : "Pending"}</span></td>
          ...
          <DialogContent className="sm:max-w-sm">
            ...
            <form className="flex flex-col gap-3" onSubmit={(e) => { e.preventDefault(); invite(); }}>
              ...
              <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Role</span>
                <select value={role} onChange={(e) => setRole(e.target.value as TeamRole)} className="h-9 rounded-md border border-border bg-card px-3 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">{ROLES.map((r) => <option key={r}>{r}</option>)}</select>
              </label>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <form className="flex flex-col gap-3" onSubmit={(e) => { e.preventDefault(); save(); }}>
          ...
          <p className="text-xs text-muted-foreground">Keys are shared in the group chat until 1Password is set up. This one stays in your browser only.</p>
          <DialogFooter>
          ...
          return (
            <li key={i.id} className="flex items-center gap-4 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted-foreground">{i.purpose}</p>
              </div>
              <span className="flex items-center gap-1.5 text-sm text-foreground/80"><StatusDot on={!!last4} />{last4 ? `•••• ${last4}` : i.kind === "key" ? "No API key" : "Not connected"}</span>
...
export function ApiKeysPanel() {
  ...
  return (
    <Card title="API keys" description="For your own scripts against the Slipstream API. Shown once, stored hashed.">
      {list.length === 0 ? (
        <p className="rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">No keys yet.</p>
      ) : (
        <ul className="divide-y divide-border rounded-lg border border-border">
          {list.map((k) => (
      ...
      <ul className="divide-y divide-border">
        {NOTIFICATIONS.map((n) => (
          <li key={n.key} className="flex items-center justify-between gap-6 py-3 first:pt-0 last:pb-0">
            <div><p className="text-sm font-medium text-foreground">{n.label}</p><p className="text-xs text-muted-foreground">{n.help}</p></div>
