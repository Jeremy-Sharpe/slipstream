  ...
  return (
    <header className="flex flex-col items-stretch justify-between gap-4 sm:flex-row sm:items-center sm:gap-6">
      <div className="flex items-center gap-4">
        <span className="flex size-[46px] items-center justify-center rounded-legacy-lg bg-icon-well text-foreground"><BarChart3 className="size-[22px]" strokeWidth={1.75} /></span>
...
function NewAnalysis() {
  ...
  return (
    <Dialog open={open} onOpenChange={reset}>
      <DialogTrigger render={<Button aria-label="New analysis" className="size-10 rounded-legacy-md p-0 text-[16px] font-medium sm:h-10 sm:w-auto sm:px-3.5" />}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <textarea
          ...
          aria-label="Question"
          className="w-full resize-none rounded-legacy-lg border border-border bg-card p-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-primary"
        />
        {state === "queued" && (
          <div className="rounded-legacy-lg border border-border bg-page p-3 text-sm">
