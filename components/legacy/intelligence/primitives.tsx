...
import type { CallRef, Outcome } from "@/lib/legacy/types/intelligence";
...
export function Section({ id, title, meta, active, children, className }: { id: string; title: string; meta?: ReactNode; active?: boolean; children: ReactNode; className?: string }) {
  return (
    <section
      id={id}
      className={cn("scroll-mt-6 rounded-legacy-xl border border-border bg-card shadow-[0_1px_2px_rgba(17,24,39,0.06)] transition-shadow", active && "ring-2 ring-primary", className)}
    >
...
export function EvidenceChip({ call }: { call: CallRef }) {
  return (
    <Link
      href={`/legacy/conversations/${call.id}`}
      className="inline-flex h-8 items-center gap-1.5 rounded-legacy-md border border-border bg-page px-2.5 text-[14px] text-foreground transition-colors hover:border-foreground/30 hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
    >
  ...
  return (
    <span
      className={cn(
        ...
        outcome === "won" ? "bg-foreground text-legacy-background" : "bg-legacy-muted text-muted-foreground",
      )}
    >
      {OUTCOME[outcome]}
    </span>
