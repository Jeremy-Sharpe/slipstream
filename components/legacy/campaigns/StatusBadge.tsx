import type { CampaignStatus, PersonStatus } from "@/lib/legacy/types/campaigns";
...
export function StatusBadge({ status, className }: { status: CampaignStatus | PersonStatus; className?: string }) {
  ...
  return (
    <span className={cn("inline-flex h-7 items-center gap-1.5 rounded-full bg-legacy-muted px-3 text-[13px] font-medium text-foreground", status === "skipped" && "text-muted-foreground", className)}>
