      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-4">
          <label className="flex flex-col gap-1.5 text-sm">
            ...
            <input autoFocus value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && create()} placeholder="Essential Eight renewals — Q4" className="h-10 rounded-legacy-lg border border-border bg-card px-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-primary" />
          </label>
          <div className="flex flex-col gap-1.5 text-sm">
            <span className="font-medium text-foreground">Source list</span>
            <div className="flex flex-col gap-1">
              {SOURCES.map((s) => (
                <button key={s} type="button" onClick={() => setSource(s)} className={cn("flex h-10 items-center gap-2.5 rounded-legacy-lg border border-border px-3 text-left text-sm text-foreground hover:bg-page", source === s && "border-primary bg-primary-soft")}>
        ...
        <DialogFooter>
          ...
          <button type="button" onClick={create} className="h-9 rounded-legacy-lg bg-primary px-3.5 text-sm font-medium text-primary-foreground hover:bg-primary/85">Create</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
