...
import type { ColumnKind, ListColumn, ListRow } from "@/lib/legacy/types/lists";
...
export function ListGrid({
  ...
  return (
    <div ref={wrapRef} className="relative h-full w-full">
      <div className="relative border border-legacy-line bg-card" style={{ width: width + 2, height: height + 2 }}>
      <DataEditor
      ...
      <button
        ...
        className="absolute flex size-6 items-center justify-center rounded text-muted-foreground hover:text-legacy-ink focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
        style={{ left: INDEX_PAD - 3 + tx, top: (HEADER_H - 24) / 2 }}
      >
        {allSelected ? <SquareCheck className="size-[18px] text-legacy-ink" strokeWidth={1.75} /> : <Square className="size-[18px]" strokeWidth={1.75} />}
      </button>
      {offsets.map((o) => {
        ...
        return (
          <button
            ...
            className="absolute flex size-8 items-center justify-center rounded-legacy-md border border-legacy-line bg-card text-legacy-ink hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none disabled:opacity-60"
            style={{ left: o.start + o.width - 42 + tx, top: (HEADER_H - 32) / 2 }}
          >
            {busy ? <span className="size-3.5 animate-spin rounded-full border-2 border-legacy-line border-t-legacy-ink" /> : <Play className="size-4" strokeWidth={1.75} />}
          </button>
        );
      })}
      {addOffset && (
        <DropdownMenu>
          <DropdownMenuTrigger
            render={<button type="button" className="absolute flex h-8 items-center gap-2 rounded-legacy-md px-2 text-[16px] font-medium whitespace-nowrap text-legacy-ink hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" style={{ left: addOffset.start + 6 + tx, top: (HEADER_H - 32) / 2 }} />}
          >
            <Plus className="size-[18px]" strokeWidth={2} /> Add column <ChevronDown className="size-4 text-legacy-ink" strokeWidth={2} />
          </DropdownMenuTrigger>
