...
import { hubspot, integrations, team as seedTeam, workspace as seedWorkspace } from "@/lib/legacy/data/settings";
import type { ApiKey, Integration, TeamMember, TeamRole } from "@/lib/legacy/types/settings";
    ...
    <Card title="Workspace" description="What the team sees at the top of every page and how numbers are shown.">
      <form className="grid max-w-xl grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); save(); }}>
        ...
        <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Timezone</span>
          <select value={ws.timezone} onChange={(e) => setWs({ ...ws, timezone: e.target.value })} className="h-10 rounded-legacy-md border border-border bg-card px-3 text-[16px] text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">
        ...
        <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Currency</span>
          <select value={ws.currency} onChange={(e) => setWs({ ...ws, currency: e.target.value })} className="h-10 rounded-legacy-md border border-border bg-card px-3 text-[16px] text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">
...
export function CrmPanel() {
  ...
  return (
    <Card title="CRM" description="Slipstream writes calls, notes, tasks and drafts into your CRM. Nothing changes without an approval.">
      <div className="flex items-start justify-between gap-6 rounded-legacy-lg border border-border p-4">
        <div className="flex items-start gap-3">
          <span className="flex size-10 items-center justify-center rounded-legacy-lg bg-icon-well text-[13px] font-semibold text-foreground">Hs</span>
          <div>
            <p className="flex items-center gap-2 text-[17px] font-semibold text-foreground">HubSpot <span className="flex items-center gap-1.5 rounded-legacy-md border border-border px-2 py-0.5 text-xs font-normal text-foreground/80"><StatusDot on={connected} />{connected ? "Connected · mock" : "Disconnected"}</span></p>
            {connected ? (
...
export function TeamPanel() {
  ...
  return (
    <Card title="Team" description="Who can see conversations and approve drafts.">
      <div className="overflow-hidden rounded-legacy-lg border border-border">
            ...
            {members.map((m) => (
              <tr key={m.id} className="border-b border-border last:border-b-0">
                ...
                <td className="px-5 py-3.5">
                  <select value={m.role} onChange={(e) => setMembers((cur) => cur.map((x) => (x.id === m.id ? { ...x, role: e.target.value as TeamRole } : x)))} aria-label={`Role for ${m.name}`} className="h-8 rounded-legacy-md border border-border bg-card px-2 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">
                ...
                <td className="px-2 py-2.5 text-right">{m.status === "pending" && <button type="button" aria-label={`Remove ${m.email}`} onClick={() => setMembers((cur) => cur.filter((x) => x.id !== m.id))} className="rounded-legacy-md p-1.5 text-muted-foreground hover:bg-legacy-muted hover:text-foreground"><Trash2 className="size-4" strokeWidth={1.75} /></button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <DialogContent className="sm:max-w-sm">
            ...
            <form className="flex flex-col gap-3" onSubmit={(e) => { e.preventDefault(); invite(); }}>
              ...
              <label className="flex flex-col gap-1.5"><span className={fieldLabel}>Role</span>
                <select value={role} onChange={(e) => setRole(e.target.value as TeamRole)} className="h-10 rounded-legacy-md border border-border bg-card px-3 text-[16px] text-foreground outline-none focus-visible:ring-2 focus-visible:ring-primary">{ROLES.map((r) => <option key={r}>{r}</option>)}</select>
              </label>
...
export function IntegrationsPanel() {
  ...
  return (
    <Card title="Integrations" description="Everything the pipeline calls. A missing key shows as not set; the screens keep working on fixture data.">
      <ul className="divide-y divide-border rounded-legacy-lg border border-border">
        {integrations.map((i) => {
          const last4 = keys[i.id];
          return (
            <li key={i.id} className="flex items-center gap-4 px-4 py-3">
              <span className="flex size-9 items-center justify-center rounded-legacy-lg bg-icon-well text-[13px] font-semibold text-foreground">{i.name.slice(0, 2)}</span>
...
export function ApiKeysPanel() {
  ...
  return (
    <Card title="API keys" description="For your own scripts against the Slipstream API. Shown once, stored hashed.">
      {list.length === 0 ? (
        <p className="rounded-legacy-lg border border-dashed border-border p-5 text-[15px] text-muted-foreground">No keys yet.</p>
      ) : (
        <ul className="divide-y divide-border rounded-legacy-lg border border-border">
          {list.map((k) => (
            <li key={k.id} className="flex items-center gap-4 px-5 py-3.5 text-[16px]">
              ...
              <button type="button" aria-label={`Revoke ${k.name}`} onClick={() => setList(list.filter((x) => x.id !== k.id))} className="rounded-legacy-md p-1.5 text-muted-foreground hover:bg-legacy-muted hover:text-foreground"><Trash2 className="size-4" strokeWidth={1.75} /></button>
            </li>
          ))}
        </ul>
      )}
          ...
          <DialogContent className="sm:max-w-md">
            ...
            {fresh ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2 rounded-legacy-md border border-border bg-page px-3 py-2 font-mono text-[13px] break-all text-foreground">{fresh}</div>
                <DialogFooter>
