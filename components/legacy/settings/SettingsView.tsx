    ...
    <div className="flex flex-col pb-10">
      <header className="flex items-center justify-between px-11 pt-[34px] pb-7">
        <div className="flex items-center gap-4">
          <span className="flex size-[46px] items-center justify-center rounded-legacy-lg bg-icon-well text-foreground"><Settings className="size-[22px]" strokeWidth={1.75} /></span>
      ...
      <div className="flex gap-10 border-t border-border px-11 pt-7">
        <nav aria-label="Settings sections" className="flex w-[220px] shrink-0 flex-col gap-0.5">
          {TABS.map((t) => (
            <button key={t.key} type="button" onClick={() => go(t.key)} aria-current={tab === t.key ? "page" : undefined} className={cn("flex h-10 items-center rounded-legacy-md px-3 text-left text-[16px] text-muted-foreground transition-colors hover:bg-legacy-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none", tab === t.key && "bg-legacy-muted font-medium text-foreground")}>
              {t.label}
            </button>
          ))}
        </nav>
