...
import { homeItems } from "@/lib/legacy/data/home";
import type { HomeItem, HomeItemType, Owner } from "@/lib/legacy/types/home";
    ...
    <section className="mt-[70px] flex flex-col">
      <div className="flex items-center justify-between px-11">
        <div className="flex items-center gap-4">
          <span className="flex size-[46px] items-center justify-center rounded-legacy-lg bg-icon-well text-foreground"><Clock className="size-[22px]" strokeWidth={1.75} /></span>
      ...
      <div className="mt-[42px] px-11 pb-[14px]">
        <div className="inline-flex h-10 items-center rounded-legacy-lg border border-border bg-card p-0.5">
          {tabs.map((t) => (
            <button key={t.key} type="button" onClick={() => setTab(t.key)} className={cn("flex h-full items-center rounded-legacy-md px-[18px] text-[16px] text-muted-foreground transition-colors hover:text-foreground", tab === t.key && "border border-border bg-card text-foreground shadow-[0_1px_2px_rgba(17,24,39,0.08)]")}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div className="border-t border-border">
        <div className="flex h-[57px] items-center gap-5 px-11">
          <DropdownMenu>
            <DropdownMenuTrigger render={<button type="button" className="flex h-8 items-center rounded-legacy-md border border-border text-[14px] text-foreground hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" />}>
          ...
          <button type="button" onClick={() => setShowTypes((v) => !v)} aria-pressed={showTypes} className={cn("flex h-8 items-center gap-1.5 rounded-legacy-md px-1 text-[15px] text-foreground hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none", showTypes && "bg-legacy-muted")}>
          ...
          {showTypes && (
            <div className="ml-2 flex items-center gap-1.5" role="radiogroup" aria-label="Type">
              {TYPES.map((t) => (
                <button key={t.key} type="button" role="radio" aria-checked={type === t.key} onClick={() => setType(t.key)} className={cn("h-7 rounded-legacy-md border px-2.5 text-[13px] transition-colors", type === t.key ? "border-primary/40 bg-primary-soft text-primary-foreground" : "border-border text-muted-foreground hover:bg-legacy-muted")}>{t.label}</button>
              ))}
            </div>
          )}
          {deleted && (
