...
import type { HomeItem, HomeItemType } from "@/lib/legacy/types/home";
    ...
    <div className="border-t border-border">
      ...
      {loading && Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className={cn("grid h-[61px] items-center border-b border-border px-11", COLS)} aria-busy="true">
          <span className="h-3.5 w-48 rounded bg-legacy-muted" /><span className="size-4 rounded bg-legacy-muted" /><span /><span className="h-3.5 w-20 rounded bg-legacy-muted" /><span className="h-3.5 w-20 rounded bg-legacy-muted" /><span className="h-3.5 w-16 rounded bg-legacy-muted" /><span className="h-3.5 w-8 rounded bg-legacy-muted" /><span />
        </div>
      ))}

      {!loading && rows.length === 0 && (
        <div className="flex h-40 items-center justify-center text-[15px] text-muted-foreground">{emptyLabel}</div>
      )}
        ...
        return (
          ...
          >
            <span className="flex min-w-0 items-center gap-2.5">
              ...
              {editing === item.id ? (
                <input
                  ...
                  aria-label="Rename"
                  className="h-8 w-80 rounded-legacy-md border border-border px-2 text-[16px] outline-none focus:ring-2 focus:ring-primary"
                />
              ) : (
            ...
            <span className="flex justify-end" onClick={(e) => e.stopPropagation()}>
              <DropdownMenu>
                <DropdownMenuTrigger render={<button type="button" aria-label={`Actions for ${item.name}`} className="flex size-8 items-center justify-center rounded-legacy-md border border-border text-foreground/70 hover:bg-legacy-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" />}>
