...
import { EMPTY_DEMO_PROOF, campaignSafety, icpClaimSafety, modelLabel, nextPresenterStep, parseStoredStep, playbackLabel, proofFooter, rateScenario, settleDemoProof, type DemoProof } from "@/lib/legacy/demo/revenue-loop";
...
const MAYA_CALL = "/legacy/conversations/call-01-northstar-labs";
const STEP_COUNT = 7;
...
export function RevenueLoop({ initialProof = EMPTY_DEMO_PROOF }: { initialProof?: DemoProof }) {
  ...
  return (
    <div className="min-h-full bg-page px-4 py-5 md:px-8 md:py-7">
      <section className="relative overflow-hidden rounded-legacy-2xl bg-foreground px-5 py-7 text-legacy-background shadow-[0_24px_70px_rgba(17,24,39,0.16)] md:px-8 md:py-8">
        ...
        <div className="relative flex flex-col items-start justify-between gap-7 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 max-w-2xl text-[16px] leading-7 text-legacy-background/70 sm:text-[17px]">Watch a buyer conversation become CRM truth, a safe follow-up, team intelligence, an evidence-backed ICP and the next campaign.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => selectStep(steps.length - 1)} className="h-11 rounded-legacy-lg border border-legacy-background/20 px-4 text-[14px] font-semibold text-legacy-background transition-colors hover:bg-legacy-background/10 focus-visible:ring-2 focus-visible:ring-legacy-background focus-visible:outline-none">Show complete loop</button>
            <button type="button" onClick={togglePlayback} className="flex h-11 items-center gap-2 rounded-legacy-lg bg-primary px-5 text-[15px] font-semibold text-primary-foreground shadow-lg transition-transform hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-legacy-background focus-visible:outline-none motion-reduce:hover:scale-100">
        ...
        <div className="relative mt-8 grid grid-cols-2 gap-4 border-t border-legacy-background/15 pt-5 lg:grid-cols-4">
      ...
      <aside className="mt-4 grid overflow-hidden rounded-legacy-xl border border-border bg-card shadow-[0_1px_3px_rgba(17,24,39,0.04)] sm:grid-cols-[auto_repeat(4,1fr)]" aria-label="Illustrative rep capacity estimate">
      ...
      <aside className="mt-3 grid gap-px overflow-hidden rounded-legacy-xl border border-border bg-border shadow-[0_1px_3px_rgba(17,24,39,0.04)] lg:grid-cols-2" aria-label="Illustrative pipeline upside scenarios">
        <ValueScenario
          icon={TrendingUp}
      ...
      <section className="mt-6 overflow-hidden rounded-legacy-2xl border border-border bg-card shadow-[0_1px_3px_rgba(17,24,39,0.05)]">
        <div className="h-1 bg-legacy-muted"><div className="h-full bg-primary transition-[width] duration-700 ease-out motion-reduce:transition-none" style={{ width: `${progress}%` }} /></div>
        ...
        <div className="grid grid-cols-1 items-center gap-6 border-t border-border bg-page px-5 py-6 lg:grid-cols-[1.2fr_1fr_auto] lg:gap-8 lg:px-7">
          <div><div className="flex flex-wrap items-center gap-2"><p className="text-[12px] font-semibold tracking-[0.12em] text-primary uppercase">{selected.eyebrow}</p><span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase", selected.verified ? "bg-emerald-100 text-emerald-800" : "bg-legacy-muted text-muted-foreground")}>{selected.provenance}</span></div><h2 className="mt-1 text-[23px] font-bold tracking-[-0.02em] text-foreground">{selected.result}</h2><p className="mt-2 max-w-xl text-[15px] leading-6 text-secondary">{selected.detail}</p></div>
          ...
          <Link href={selected.href} className="flex h-10 w-fit items-center gap-2 rounded-legacy-lg border border-border bg-card px-4 text-[14px] font-semibold text-foreground shadow-sm transition-colors hover:border-foreground/30 hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">{selected.cta}<ArrowRight className="size-4" /></Link>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border bg-card px-5 py-3">
          <div className="flex items-center gap-2"><button type="button" disabled={active <= 0} onClick={() => selectStep(active - 1)} className="flex h-8 items-center gap-1 rounded-legacy-md border border-border px-2.5 text-xs font-medium disabled:opacity-40"><ArrowLeft className="size-3.5" /> Previous</button><button type="button" disabled={active >= steps.length - 1} onClick={() => selectStep(Math.max(0, active + 1))} className="flex h-8 items-center gap-1 rounded-legacy-md border border-border px-2.5 text-xs font-medium disabled:opacity-40">Next <ArrowRight className="size-3.5" /></button></div>
...
function ProofPill({ label, value, live, mono = false }: { label: string; value: string; live: boolean; mono?: boolean }) {
  return <div className="flex min-w-0 items-center gap-3"><span className={cn("size-2 shrink-0 rounded-full", live ? "bg-primary shadow-[0_0_12px_var(--primary)]" : "bg-legacy-background/25")} aria-hidden /><span className="min-w-0"><span className="block text-[11px] tracking-wide text-legacy-background/45 uppercase">{label}</span><span className={cn("mt-0.5 block truncate text-[14px] font-medium text-legacy-background", mono && "font-mono")} title={value}>{value}</span></span></div>;
}
