...
import type { DemoEmailThread } from "@/lib/legacy/data/emails";
...
export function EmailConversationDetail({ thread }: { thread: DemoEmailThread }) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-page">
      <header className="border-b border-legacy-line bg-card px-6 py-5">
        <Link href="/legacy" className="mb-4 inline-flex items-center gap-2 text-[13px] text-muted-foreground hover:text-legacy-ink">
          <ArrowLeft className="size-4" /> Conversations
        </Link>
        <div className="flex items-start justify-between gap-5">
          <div className="flex min-w-0 items-start gap-3">
            <div className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-legacy-lg border border-legacy-line bg-page">
              <Mail className="size-4 text-primary" />
            </div>
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-2">
                <span className="rounded-full border border-legacy-line px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Email thread</span>
                <span className="text-[12px] text-muted-foreground">{messages.length} messages</span>
              </div>
              <h1 className="truncate text-xl font-semibold tracking-tight text-legacy-ink">{thread.subject}</h1>
              <p className="mt-1 text-[13px] text-muted-foreground">{thread.contact} · {thread.company}</p>
            </div>
          </div>
          <Button onClick={() => void runPipeline()} disabled={isBusy} className="h-9 shrink-0 rounded-legacy-md px-4 text-[13px]">
      ...
      <div className="mx-6 mt-5 flex items-center gap-3 rounded-legacy-lg border border-legacy-line bg-card px-4 py-3 shadow-[0_1px_2px_rgba(17,24,39,0.04)]">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[14px] font-medium text-legacy-ink">
      ...
      <main className="grid grid-cols-[minmax(0,1.7fr)_minmax(340px,1fr)] gap-6 p-6">
        <section className="space-y-4">
          {messages.map((message) => (
            <article key={"id" in message ? message.id : message.source_external_id} className={`rounded-legacy-xl border border-legacy-line bg-card p-5 shadow-[0_1px_2px_rgba(17,24,39,0.04)] ${message.direction === "outbound" ? "ml-10" : "mr-10"}`}>
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <p className="text-[14px] font-semibold text-legacy-ink">{senderLabel(message)}</p>
                ...
                <div className="text-right">
                  <span className="rounded-full border border-legacy-line px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">{message.direction}</span>
                  <p className="mt-1 text-[11px] text-muted-foreground">{when.format(new Date(message.occurred_at))}</p>
                </div>
              </div>
              <p className="mb-3 text-[13px] font-medium text-legacy-ink">{message.subject}</p>
        ...
        <aside className="space-y-5">
          <section className="rounded-legacy-xl border border-legacy-line bg-card p-5 shadow-[0_1px_2px_rgba(17,24,39,0.05)]">
            <div className="mb-4 flex items-center gap-2"><Sparkles className="size-4 text-primary" /><h2 className="text-[14px] font-semibold text-legacy-ink">CRM-shaped mapping</h2></div>
            <dl className="grid grid-cols-[100px_1fr] gap-x-3 gap-y-3 text-[13px]">
              <dt className="text-muted-foreground">Contact</dt><dd className="font-medium text-legacy-ink">{thread.contact}</dd>
              <dt className="text-muted-foreground">Email</dt><dd className="break-all text-legacy-ink">{thread.messages[0].sender.email}</dd>
              <dt className="text-muted-foreground">Company</dt><dd className="text-legacy-ink">{thread.company}</dd>
              <dt className="text-muted-foreground">Role</dt><dd className="text-legacy-ink">{thread.title}</dd>
              <dt className="text-muted-foreground">Deal activity</dt><dd className="text-legacy-ink">{messages.length} emails captured</dd>
            </dl>
            <p className="mt-4 border-t border-legacy-line pt-3 text-[11px] text-muted-foreground">{isLive ? "Live API confirmed the provider-scoped email records and primary contact email. Company and role are labelled demo account context." : "Labelled demo account context; run live to persist the email records and primary contact mapping."}</p>
          </section>

          <section className="rounded-legacy-xl border border-legacy-line bg-card p-5 shadow-[0_1px_2px_rgba(17,24,39,0.05)]">
            <div className="mb-4 flex items-center justify-between"><h2 className="text-[14px] font-semibold text-legacy-ink">Grounded reply draft</h2>{draft && <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-primary">Live</span>}</div>
            {draft ? (
              <div className="space-y-3">
                <div className="rounded-legacy-md border border-legacy-line bg-page px-3 py-2 text-[13px] font-medium text-legacy-ink">{draft.subject}</div>
                <div className="whitespace-pre-wrap rounded-legacy-md border border-legacy-line bg-page px-3 py-3 text-[13px] leading-5 text-foreground">{draft.body}</div>
              ...
              <div className="rounded-legacy-lg border border-dashed border-legacy-line px-4 py-8 text-center text-[13px] text-muted-foreground">Run the live email pipeline to draft a reply from the complete thread.</div>
            )}
          </section>
        </aside>
      </main>
