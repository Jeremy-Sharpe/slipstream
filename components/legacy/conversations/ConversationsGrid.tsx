...
import type { Conversation, ConversationStatus } from "@/lib/legacy/types";
...
export function ConversationsGrid({ rows, onSelectionCount }: { rows: Conversation[]; onSelectionCount?: (n: number) => void }) {
  ...
  const open = useCallback((row: number) => { const r = rows[row]; if (r) router.push(`/legacy/conversations/${r.id}`); }, [rows, router]);
