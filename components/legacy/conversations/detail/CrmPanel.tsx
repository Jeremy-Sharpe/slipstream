...
import type { CallRecord, TimelineEntry } from "@/lib/legacy/types/calls";
...
const inputCls = "h-10 w-full rounded-legacy-md border border-legacy-line bg-card px-3 text-[15px] text-legacy-ink outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-70";
    ...
    <aside className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-[20px] font-bold tracking-[-0.02em] text-legacy-ink">HubSpot</h2>
        </div>
        <span className={cn("rounded-legacy-md px-2.5 py-1 text-[12px] font-semibold tracking-wide uppercase", synced ? "bg-foreground text-legacy-background" : "bg-primary-soft text-legacy-ink")}>{synced ? "Synced" : "Review"}</span>
      </div>

      <div className="flex items-center gap-3 rounded-legacy-lg bg-primary-soft px-4 py-3">
        <Sparkles className="size-[18px] shrink-0 text-primary" strokeWidth={2} />
        <div className="text-[15px]">
          <div className="font-medium text-legacy-ink">Auto-filled from conversation</div>
          <div className="text-[14px] text-ink-2">Hover a field to see where it came from</div>
        </div>
      </div>

      <div className="grid grid-cols-[40px_1fr_32px] items-center gap-3 rounded-legacy-lg border border-legacy-line p-4">
        <span className="flex size-10 items-center justify-center rounded-legacy-md bg-avatar text-[13px] font-semibold text-legacy-ink">{initials(call.prospect)}</span>
        <div className="min-w-0">
          <div className="truncate text-[16px] font-medium text-legacy-ink">{call.prospect}</div>
          <div className="truncate text-[14px] text-muted-foreground">{x.contact.role.value} · {call.company}</div>
        </div>
        {call.domain ? <a href={`https://${call.domain}`} target="_blank" rel="noreferrer" className="flex size-8 items-center justify-center rounded-legacy-md text-muted-foreground hover:bg-legacy-muted hover:text-foreground" aria-label="Open website"><ExternalLink className="size-4" strokeWidth={1.75} /></a> : <span />}
      </div>
        ...
        {x.promises.length > 0 && (
          <div className="grid gap-1.5">
            ...
            <ul className="grid gap-1.5">
              {x.promises.map((p, i) => (
                <li key={i} onMouseEnter={() => onHover(p.span)} onMouseLeave={() => onHover(null)} className="flex items-start gap-2 rounded-legacy-md border border-legacy-line px-3 py-2.5 text-[15px] text-ink-2">
          ...
          <div className="grid gap-1.5">
            ...
            <ul className="grid gap-1.5">
              {call.riskFlags.map((r) => (
                <li key={r.turnIndex} onMouseEnter={() => onHover(r.turnIndex)} onMouseLeave={() => onHover(null)} className="rounded-legacy-md border border-legacy-line px-3 py-2.5 text-[15px] text-ink-2">
                  <span className="mr-1.5 rounded-full bg-primary-soft px-2 py-px text-[12px] font-medium text-legacy-ink">{r.kind}</span>“{r.text}”
                </li>
              ))}
            </ul>
          </div>
      ...
      <Button className="h-11 w-full rounded-legacy-md text-[16px] font-medium" onClick={onSync} disabled={synced}>
      ...
      <div className="border-t border-legacy-line pt-5">
        <h3 className="text-[17px] font-semibold text-legacy-ink">Timeline</h3>
          ...
          {timeline.map((t, i) => {
            ...
            return (
              <li key={i} className="grid grid-cols-[32px_1fr] gap-3">
                <span className="flex size-8 items-center justify-center rounded-legacy-md bg-legacy-muted text-ink-2"><Icon className="size-4" strokeWidth={1.75} /></span>
                <div>
                  <div className="text-[15px] font-medium text-legacy-ink">{t.title}</div>
      ...
      <Link href="/legacy/settings" className="text-[14px] text-muted-foreground hover:text-foreground">HubSpot connection settings →</Link>
    </aside>
  );
}
