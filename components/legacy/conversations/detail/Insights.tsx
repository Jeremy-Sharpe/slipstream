    ...
    <Card className="border-l-2 border-l-primary">
      <div className="px-6 py-5">
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-2 text-[17px] font-semibold text-legacy-ink"><Sparkles className="size-[18px] text-primary" strokeWidth={2} /> Conversation intelligence</span>
          <span className="rounded-legacy-md bg-legacy-muted px-2.5 py-1 text-[13px] text-muted-foreground">{Math.round(call.extraction.deal.outcome.confidence * 100)}% confidence</span>
        </div>
        <p className="mt-3 text-[16px] leading-6 text-ink-2">{call.summary}</p>
        <dl className="mt-5 grid grid-cols-3 divide-x divide-legacy-line border-t border-legacy-line pt-4">
          {[["Sentiment", sentiment], ["Next step", next], ["Primary risk", risk]].map(([k, v], i) => (
            <div key={k} className={cn("min-w-0", i > 0 && "pl-4")}>
              ...
              <dd className="mt-1 truncate text-[15px] font-medium text-legacy-ink" title={v}>{v}</dd>
            </div>
          ))}
        </dl>
      </div>
...
export function ScorecardCard({ call, onHover, source, canGenerate, generating, error, onGenerate }: { call: CallRecord; onHover: (span: number | null) => void; source: string; canGenerate: boolean; generating: boolean; error?: string; onGenerate: () => void }) {
  ...
  return (
    <Card title={<><Gauge className="size-4 text-muted-foreground" strokeWidth={1.75} /> Scorecard <span className="rounded-legacy-md bg-legacy-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">{source}</span></>} aside={<span className="flex items-center gap-3"><span className="text-[14px] text-muted-foreground">{rows.filter((r) => r.ok).length} / 4</span>{canGenerate && <Button variant="outline" className="h-8 text-[12px]" disabled={generating} onClick={onGenerate}>{generating ? "Scoring…" : "Generate live scorecard"}</Button>}</span>}>
          ...
          return (
            <li key={r.label} className="grid grid-cols-[220px_1fr] gap-5 px-6 py-4" onMouseEnter={() => onHover(r.span)} onMouseLeave={() => onHover(null)}>
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div className="text-[15px] font-medium text-legacy-ink">{r.label}</div>
      ...
      {error && <p role="alert" className="border-t border-legacy-line px-6 py-3 text-[13px] text-destructive">Live scoring unavailable: {error}. Previous {source.startsWith("Live") ? "live scorecard" : "labelled evaluation"} retained.</p>}
    </Card>
  );
}
...
export function FollowUpDraft({ call, onApprove, approved, locked, busy }: { call: CallRecord; onApprove: () => void; approved: boolean; locked: boolean; busy: boolean }) {
  ...
  return (
    <Card title={<><Mail className="size-4 text-muted-foreground" strokeWidth={1.75} /> Follow-up drafted from this call</>} aside={<span className="rounded-legacy-md bg-legacy-muted px-2 py-0.5 text-xs text-muted-foreground">{approved ? "Approved" : "Ready to review"}</span>}>
      <div className="flex h-11 items-center gap-3 border-b border-legacy-line px-6 text-[15px]">
        <span className="text-muted-foreground">To</span>
        <span className="text-legacy-ink">{call.prospect} &lt;{call.extraction.contact.email.value}&gt;</span>
      </div>
      <input value={subject} onChange={(e) => setSubject(e.target.value)} disabled={approved || locked} className="h-12 w-full border-b border-legacy-line bg-transparent px-6 text-[16px] font-medium text-legacy-ink outline-none focus-visible:bg-page disabled:opacity-70" aria-label="Subject" />
      ...
      <footer className="flex items-center justify-between border-t border-legacy-line px-5 py-3.5">
        ...
        {approved ? <span className="flex items-center gap-1.5 text-[15px] font-medium text-legacy-ink"><Check className="size-4" strokeWidth={2} /> Approved · logged</span> : <Button className="h-10 rounded-legacy-md px-4 text-[15px] font-medium" onClick={onApprove} disabled={busy}><Check className="size-4" strokeWidth={2} /> {busy ? "Approving…" : locked ? "Approve" : "Generate live draft"}</Button>}
      </footer>
    </Card>
  );
}
