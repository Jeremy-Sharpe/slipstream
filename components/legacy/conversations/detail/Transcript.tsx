...
import type { CallRecord, Turn } from "@/lib/legacy/types/calls";
...
export function Card({ id, title, aside, children, className }: { id?: string; title?: React.ReactNode; aside?: React.ReactNode; children: React.ReactNode; className?: string }) {
  return (
    <section id={id} className={cn("scroll-mt-6 rounded-legacy-xl border border-legacy-line bg-card shadow-[0_1px_2px_rgba(17,24,39,0.06)]", className)}>
      {title && (
        <header className="flex h-14 items-center justify-between border-b border-legacy-line px-6">
          <h3 className="flex items-center gap-2 text-[17px] font-semibold text-legacy-ink">{title}</h3>
          {aside}
        </header>
      )}
      {children}
...
export function AudioPlayer({ duration }: { duration: number }) {
  ...
  return (
    <div className="flex h-[72px] items-center gap-4 rounded-legacy-xl border border-legacy-line bg-card px-5 shadow-[0_1px_2px_rgba(17,24,39,0.06)]">
      <button type="button" onClick={() => setPlaying((p) => !p)} aria-label={playing ? "Pause" : "Play"} className="flex size-10 shrink-0 items-center justify-center rounded-full bg-foreground text-legacy-background focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:outline-none">
      ...
      <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-legacy-muted" role="progressbar" aria-valuenow={Math.round(pct)}>
...
export function Transcript({ call, highlight }: { call: CallRecord; highlight: number | null }) {
  ...
  return (
    <Card id="transcript" title="Transcript" aside={<span className="text-[14px] text-muted-foreground">{call.turns.length} turns · diarised</span>}>
      <ol className="divide-y divide-legacy-line">
...
function TurnRow({ turn, active, risky }: { turn: Turn; active: boolean; risky: boolean }) {
  ...
  return (
    <li id={`turn-${turn.index}`} className={cn("grid grid-cols-[36px_1fr_auto] gap-4 px-6 py-4 transition-colors", active && "bg-primary-soft")}>
      <span className={cn("flex size-9 items-center justify-center rounded-legacy-md text-[12px] font-semibold", rep ? "bg-legacy-muted text-ink-2" : "bg-foreground text-legacy-background")}>{initials(turn.name)}</span>
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[15px] font-semibold text-legacy-ink">{turn.name}</span>
          {risky && <span className="rounded-full bg-primary-soft px-2 py-px text-[12px] font-medium text-legacy-ink">Coach flag</span>}
        </div>
