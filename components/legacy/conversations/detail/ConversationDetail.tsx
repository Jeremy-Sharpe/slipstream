...
import { patchConversation, useConversations } from "@/lib/legacy/store/conversations";
...
export function ConversationDetail({ call, others }: { call: CallRecord; others: Other[] }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] flex-col bg-page">
      ...
      <div className="mx-3 mt-4 flex flex-wrap items-center gap-3 rounded-legacy-lg border border-legacy-line bg-card px-4 py-3 shadow-[0_1px_2px_rgba(17,24,39,0.04)] sm:mx-6 sm:mt-5">
        ...
        <Button variant="outline" className="h-9 rounded-legacy-md px-3 text-[13px]" onClick={() => void runPipeline().catch(() => undefined)} disabled={rerunning || approving}>
          {pipelineStatus === "live" ? "Run again" : "Run verified API demo"}
        </Button>
      </div>
      <div className="grid flex-1 grid-cols-1 gap-4 px-3 py-4 lg:grid-cols-[minmax(0,2fr)_minmax(360px,1fr)] lg:gap-6 lg:px-6 lg:py-6">
        ...
        <div id="crm-writeback" className="scroll-mt-6 self-start rounded-legacy-xl border border-legacy-line bg-card p-4 shadow-[0_1px_2px_rgba(17,24,39,0.06)] sm:p-6 lg:sticky lg:top-6">
