...
import { conversations as fixtures } from "@/lib/legacy/data/conversations";
import { addConversation } from "@/lib/legacy/store/conversations";
...
export function AddCallDialog() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button className="h-9 rounded-legacy-md px-3.5 text-[16px] font-medium" />}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div role="radiogroup" className="max-h-56 overflow-y-auto rounded-legacy-md border border-border">
            {fixtures.map((f) => {
              ...
              return (
                <button
                  ...
                  className={cn("flex w-full items-center gap-3 border-b border-border px-3 py-2 text-left text-sm last:border-b-0 hover:bg-legacy-muted", on && "bg-legacy-muted")}
                >
          ...
          <label className={cn("flex h-10 cursor-pointer items-center gap-2 rounded-legacy-md border border-dashed border-border px-3 text-sm text-muted-foreground hover:bg-legacy-muted", file && "border-solid text-foreground")}>
