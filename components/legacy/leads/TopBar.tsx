...
export function TopBar({ searching, disabled, onRunSearch, brief, onBriefChange, integrations }: {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center border-b border-legacy-line bg-card pr-4 pl-3">
      <Link href="/legacy" aria-label="Back to conversations" className="flex size-9 shrink-0 items-center justify-center rounded-legacy-md border border-legacy-line text-muted-foreground transition-colors hover:bg-legacy-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
        <ArrowLeft className="size-[18px]" strokeWidth={1.75} />
      </Link>

      <DropdownMenu>
        <DropdownMenuTrigger render={<button type="button" className="ml-6 flex items-center gap-2 rounded-legacy-md px-1 py-1 text-[17px] font-semibold text-legacy-ink hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" />}>
          ...
          <DropdownMenuGroup>
            ...
            {SURFACES.map(({ href, label, icon: Icon }) => (
              <DropdownMenuItem key={href} onClick={() => router.push(href)}>
                ...
                {href === "/legacy/leads" && <Check className="size-4" strokeWidth={2} />}
              </DropdownMenuItem>
            ))}
          </DropdownMenuGroup>
        </DropdownMenuContent>
      ...
      <div className="ml-auto flex items-center gap-2.5">
        <button
          ...
          className="mr-6 flex h-9 items-center gap-2 rounded-legacy-md bg-primary px-3.5 text-base font-medium text-primary-foreground transition-colors hover:bg-primary/85 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:outline-none disabled:opacity-70"
        >
          {searching ? <Loader2 className="size-[18px] animate-spin" strokeWidth={2} /> : <Sparkles className="size-[18px]" strokeWidth={2} />}
          {searching ? "Searching…" : "Run search"}
        </button>

        <IconButton aria-label="Settings" render={<Link href="/legacy/settings" />}>
        ...
        <button
          ...
          className="ml-2 flex h-9 items-center gap-2 rounded-legacy-md border border-legacy-line px-3.5 text-base text-legacy-ink transition-colors hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
        >
      ...
      <Dialog open={briefOpen} onOpenChange={setBriefOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <textarea
            ...
            rows={9}
            className="w-full resize-y rounded-legacy-md border border-legacy-line bg-card p-3 text-sm leading-relaxed text-legacy-ink outline-none focus-visible:ring-2 focus-visible:ring-primary"
          />
          <DialogFooter>
      ...
      <Dialog open={shortcutsOpen} onOpenChange={setShortcutsOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <dl className="flex flex-col divide-y divide-legacy-line">
            {SHORTCUTS.map(([key, what]) => (
              <div key={key} className="flex items-center justify-between py-2 text-sm">
                <dt className="text-muted-foreground">{what}</dt>
                <dd><kbd className="rounded-legacy-md border border-legacy-line bg-legacy-muted px-1.5 py-0.5 font-mono text-xs text-legacy-ink">{key}</kbd></dd>
              </div>
            ))}
          </dl>
        </DialogContent>
