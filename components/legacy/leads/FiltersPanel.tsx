...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/legacy/ui/dropdown-menu";
...
function Section({ title, open, onToggle, trailing, children }: { title: string; open: boolean; onToggle: () => void; trailing?: ReactNode; children?: ReactNode }) {
  return (
    <section className="border-b border-legacy-line">
      <button type="button" onClick={onToggle} aria-expanded={open} className="flex h-[58px] w-full items-center gap-3.5 pr-[22px] pl-8 text-left text-base font-semibold text-legacy-ink focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary focus-visible:outline-none">
    ...
    <div className="flex flex-col gap-2">
      {items.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {items.map((v) => (
            <span key={v} className="inline-flex h-8 items-center gap-1.5 rounded-legacy-md border border-legacy-line bg-legacy-muted px-2.5 text-sm text-legacy-ink">
              {v}
      ...
      {editing ? (
        <input
          ...
          aria-label={label}
          className="h-9 w-full rounded-legacy-md border border-primary px-3.5 text-base text-legacy-ink outline-none ring-2 ring-primary/30 placeholder:text-muted-foreground"
        />
      ) : (
        <button type="button" onClick={() => setEditing(true)} className="flex h-9 w-full items-center justify-center gap-2 rounded-legacy-md border border-legacy-line text-base text-legacy-ink hover:bg-legacy-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
...
function NumberField({ label, help, value, min, onChange }: { label: string; help?: string; value: number; min: number; onChange: (n: number) => void }) {
  return (
    <label className="flex flex-col gap-2">
      <span className="text-base font-semibold text-legacy-ink">{label}</span>
      ...
      <input
        ...
        className="h-10 w-full rounded-legacy-lg border border-legacy-line px-3.5 text-base text-legacy-ink tabular-nums outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"
      />
    </label>
  );
}
...
export function FiltersPanel({ state, onChange, brief, onHide }: { state: FilterState; onChange: (next: FilterState) => void; brief: string; onHide: () => void }) {
  ...
  return (
    <aside className="flex w-[605px] shrink-0 flex-col border-r border-legacy-line bg-card">
      <header className="flex h-[62px] shrink-0 items-center border-b border-legacy-line pr-[22px] pl-6">
        <IconButton aria-label="Hide filters" onClick={onHide}>
          <PanelLeft className="size-[18px]" strokeWidth={1.75} />
        </IconButton>
        <h2 className="ml-3 text-[17px] font-semibold text-legacy-ink">Filters</h2>
        {saved && <span className="ml-2 text-sm text-muted-foreground" aria-live="polite">Saved</span>}
        <div className="ml-auto flex items-center gap-2.5">
          <div className="flex h-9 items-center rounded-legacy-md border border-legacy-line" role="group" aria-label="Panel view">
            <button type="button" aria-pressed={view === "filters"} aria-label="Filters view" onClick={() => setView("filters")} className={cn("flex h-full w-[50px] items-center justify-center rounded-l-[5px] text-muted-foreground hover:bg-legacy-muted hover:text-foreground", view === "filters" && "bg-legacy-muted text-legacy-ink")}><SlidersHorizontal className="size-[18px]" strokeWidth={1.75} /></button>
            <span className="h-full w-px bg-legacy-line" />
            <button type="button" aria-pressed={view === "code"} aria-label="Code view" onClick={() => setView("code")} className={cn("flex h-full w-[50px] items-center justify-center rounded-r-[5px] text-muted-foreground hover:bg-legacy-muted hover:text-foreground", view === "code" && "bg-legacy-muted text-legacy-ink")}><Code2 className="size-[18px]" strokeWidth={1.75} /></button>
          </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger render={<button type="button" aria-label="Save filters" className="flex h-9 items-center gap-2 rounded-legacy-md border border-legacy-line px-3 text-muted-foreground hover:bg-legacy-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" />}>
      ...
      {view === "code" ? (
        <pre className="min-h-0 flex-1 overflow-auto p-[22px] font-mono text-[13px] leading-relaxed whitespace-pre-wrap text-legacy-ink">{code}</pre>
      ) : (
        <div className="min-h-0 flex-1 overflow-y-auto">
          <Section
            ...
            trailing={filterCount > 0 && <span className="text-[15px] font-normal text-legacy-ink">{filterCount} {filterCount === 1 ? "filter" : "filters"}</span>}
          >
            <div className="px-[22px] pb-[22px]">
              <label className="flex h-11 items-center gap-2.5 rounded-legacy-lg border border-legacy-line px-4 text-muted-foreground focus-within:ring-2 focus-within:ring-primary">
                <Search className="size-4" strokeWidth={1.75} />
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search filters" className="w-full bg-transparent text-base text-legacy-ink outline-none placeholder:text-muted-foreground" />
                  ...
                  {showSuggested && (
                    <div className="mt-2 flex flex-wrap gap-x-2 gap-y-1.5">
                      ...
                      {suggested.map((f) => {
                        const Icon = f.icon;
                        return (
                          <button key={f.key} type="button" onClick={() => add(f.key)} className="flex h-8 items-center gap-2 rounded-legacy-lg border border-primary/35 bg-primary-soft px-3 text-[15px] text-primary-foreground transition-colors hover:border-primary focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
              ...
              {active.length > 0 && (
                <div className="mt-5">
                  <div className="mb-2.5 flex items-center gap-2 text-base text-legacy-ink">
            ...
            <div className="flex flex-col gap-3.5 px-[22px] pb-[18px]">
              <div role="radiogroup" aria-label="Target companies" className="flex flex-col gap-4 pt-0.5">
                {TARGET_MODES.map((m) => {
                  ...
                  return (
                    <button key={m.key} type="button" role="radio" aria-checked={on} onClick={() => patch({ targetMode: m.key })} className="flex items-center gap-3 text-left text-base text-legacy-ink focus-visible:outline-none">
