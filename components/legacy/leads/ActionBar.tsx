...
export function ActionBar({ pending, onApproveAll, leads }: { pending: number; onApproveAll: () => void; leads: Lead[] }) {
  return (
    <div className="flex h-[65px] shrink-0 items-center justify-end border-t border-legacy-line bg-card pr-[22px]">
      <div className="flex h-10 items-stretch">
        <button type="button" onClick={onApproveAll} disabled={pending === 0} className={`${BTN} rounded-l-legacy-md pr-3.5 pl-4`}>
          Approve all drafts
        </button>
        <DropdownMenu>
          <DropdownMenuTrigger render={<button type="button" aria-label="More actions" className={`${BTN} rounded-r-legacy-md pr-3 pl-1.5`} />}>
