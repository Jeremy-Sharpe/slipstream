...
export function LeadTable({ leads, onChange, onApprove, onCreateDraft, busyLeadId }: { leads: Lead[]; onChange: (next: Lead[]) => void; onApprove: (lead: Lead) => void; onCreateDraft: (lead: Lead) => void; busyLeadId: string | null }) {
  ...
  if (leads.length === 0) {
    return (
      <div className="mx-[22px] mb-[22px] flex-1 rounded-legacy-lg border border-legacy-line bg-card">
  ...
  return (
    <div className="mx-[22px] mb-[22px] min-h-0 flex-1 overflow-hidden rounded-legacy-lg border border-legacy-line bg-card">
      <div className="h-full overflow-auto">
        <table className="w-full table-fixed border-collapse">
          ...
          <thead className="sticky top-0 z-10 bg-card">
            <tr>
              {COLUMNS.map((c, i) => (
                <th key={i} scope="col" className={cn("h-[46px] border-r border-b border-legacy-line text-left align-middle", c.glyph ? "px-4" : "px-0")}>
                  {c.glyph && (
                    <span className="flex items-center gap-2.5 overflow-hidden">
                      <ColumnGlyph glyph={c.glyph} />
                      <span className="truncate text-base font-semibold text-legacy-ink">{c.label}</span>
                    </span>
                  )}
                </th>
              ))}
              <th className="h-[46px] border-b border-legacy-line" />
            </tr>
          </thead>
          <tbody>
            {leads.map((lead, i) => (
