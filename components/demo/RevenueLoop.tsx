...
export function RevenueLoop({ initialProof = EMPTY_DEMO_PROOF }: { initialProof?: DemoProof }) {
  ...
  const steps = useMemo<LoopStep[]>(() => {
    ...
    return [
      { eyebrow: "01 · Listen", title: "One sales call", result: "Maya names the deadline", detail: "The recorded buyer example explains that her research note load roughly doubles when a new fund launches in November.", impact: "The conversation becomes structured input instead of disappearing into a recorder.", provenance: "Recorded Maya example", verified: false, href: `${MAYA_CALL}#transcript`, cta: "Open transcript", icon: Volume2 },
      ...
      { eyebrow: "03 · Respond", title: "Safe follow-up", result: "Risky guarantee removed", detail: "The recorded draft keeps the agreed next step, a dated walkthrough of the research note drafting pilot, but refuses to repeat an unsupported accuracy promise.", impact: "The rep gets speed without turning model fluency into commercial risk.", provenance: "Recorded Maya example", verified: false, href: `${MAYA_CALL}#follow-up-draft`, cta: "Review exact draft", icon: MailCheck },
