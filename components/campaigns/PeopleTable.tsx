...
const COLS = "grid-cols-[minmax(200px,1fr)_minmax(160px,220px)_80px_110px_100px_48px]";
  ...
  return (
    <div className="min-w-[700px] flex-1">
      <div className="flex h-12 items-center justify-between border-b border-border px-6">
        ...
        <span className="text-xs text-muted-foreground">{campaign.people.length} enrolled</span>
      </div>
      <div className={cn("grid h-11 items-center border-b border-border px-6 text-[13px] font-semibold text-foreground", COLS)}>
      ...
      {campaign.people.length === 0 && (
        <div className="flex h-48 flex-col items-center justify-center gap-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">Add people from a lead list to draft their sequences.</p>
        </div>
      )}
      {campaign.people.map((p) => (
        <div
          ...
          className={cn("grid h-[52px] cursor-pointer items-center border-b border-border px-6 text-sm text-foreground transition-colors hover:bg-page focus-visible:bg-page focus-visible:outline-none", COLS, selectedId === p.id && "bg-page")}
        >
          <span className="flex min-w-0 items-center gap-2.5">
            <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-avatar text-[11px] font-medium text-foreground">{initials(p.name)}</span>
