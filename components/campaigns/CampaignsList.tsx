...
const COLS = "grid-cols-[minmax(0,1fr)_110px_90px_100px_90px_90px_120px_130px_56px]";
    ...
    <section className="flex flex-col pt-9">
      <div className="flex items-center justify-between px-9">
        <div className="flex items-center gap-4">
          <span className="flex size-10 items-center justify-center rounded-lg bg-icon-well text-foreground"><Send className="size-5" strokeWidth={1.75} /></span>
        ...
        <div className="flex items-center gap-2.5">
          <label className="flex h-9 w-[240px] items-center gap-2 rounded-lg border border-border bg-card px-3 text-muted-foreground focus-within:ring-2 focus-within:ring-primary">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Search campaigns" className="w-full bg-transparent text-sm text-foreground outline-none" />
          </label>
          <NewCampaignDialog />
        </div>
      </div>
      ...
      <div className="mt-8 px-9 pb-4">
        <div className="inline-flex h-9 items-center rounded-lg border border-border bg-muted/70 p-0.5">
          {TABS.map((t) => (
            <button key={t.key} type="button" onClick={() => setTab(t.key)} className={cn("flex h-full items-center gap-1.5 rounded-[5px] px-3.5 text-sm text-muted-foreground transition-colors hover:text-foreground", tab === t.key && "border border-border bg-card font-medium text-foreground shadow-xs")}>
              {t.label}
              <span className="text-xs tabular-nums text-muted-foreground">{t.key === "all" ? campaigns.length : campaigns.filter((c) => c.status === t.key).length}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {deleted && (
        <div className="mx-9 mb-3 flex h-10 items-center justify-between rounded-lg border border-border bg-page px-3 text-sm text-foreground">
      ...
      <TooltipProvider>
        <div className="border-t border-border">
          <div className={cn("grid h-12 items-center border-b border-border px-9 text-[15px] font-semibold text-foreground", COLS)}>
          ...
          {!loading && visible.map((c) => (
            ...
            >
              ...
              <span className="flex items-center gap-2"><span className="size-3.5 rounded-full bg-foreground" />{c.owner}</span>
              <span className="text-muted-foreground">{ago(c.updatedMinutesAgo)}</span>
