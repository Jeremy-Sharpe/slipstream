                  ...
                  {open ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => setEditing(null)} className="ml-auto h-7 rounded-md bg-primary px-2.5 text-xs font-medium text-primary-foreground hover:bg-primary/85">Done</button>
                      </div>
                    </div>
                  ) : (
                    <>
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-border bg-page p-3">
            <div className="flex gap-1">
              {(["email", "linkedin"] as StepChannel[]).map((c) => (
                <button key={c} type="button" onClick={() => setChannel(c)} className={cn("h-8 flex-1 rounded-md border border-border text-[13px] text-muted-foreground hover:text-foreground", channel === c && "bg-card font-medium text-foreground")}>{c === "email" ? "Email" : "LinkedIn"}</button>
              ))}
            </div>
            <label className="flex items-center gap-1.5 text-xs text-muted-foreground">Delay
            ...
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setAdding(false)} className="h-8 rounded-md border border-border px-2.5 text-xs text-foreground hover:bg-muted">Cancel</button>
              <button type="button" onClick={add} className="h-8 rounded-md bg-primary px-2.5 text-xs font-medium text-primary-foreground hover:bg-primary/85">Add step</button>
            </div>
          </div>
        ) : (
          <button type="button" onClick={() => setAdding(true)} className="flex h-9 w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-border text-sm text-muted-foreground hover:border-foreground/40 hover:text-foreground">
