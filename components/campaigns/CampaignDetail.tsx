  ...
  return (
    <div className="flex h-[calc(100vh-64px)] flex-col">
      <nav aria-label="Breadcrumb" className="flex h-16 shrink-0 items-center gap-2 border-b border-border px-5 text-[17px]">
        <Link href="/home" className="flex items-center gap-2 rounded-md px-1.5 py-1 text-foreground hover:bg-muted"><Folder className="size-[18px] text-foreground" strokeWidth={1.75} />Home</Link>
        <ChevronRight className="size-4 text-muted-foreground" strokeWidth={2} />
        <Link href="/campaigns" className="flex items-center gap-2 rounded-md px-1.5 py-1 text-foreground hover:bg-muted"><Send className="size-[18px] text-foreground" strokeWidth={1.75} />Campaigns</Link>
        <ChevronRight className="size-4 text-muted-foreground" strokeWidth={2} />
        {renaming ? (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger render={<button type="button" className="flex items-center gap-2 rounded-md px-1.5 py-1 font-semibold text-foreground hover:bg-muted" />}>
      ...
      <div className="flex h-[72px] shrink-0 items-center gap-3 border-b border-border px-5">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button type="button" onClick={() => campaignActions.toggleStatus(campaign.id)} className="flex h-10 items-center gap-2 rounded-md border border-border px-3.5 text-[15px] font-medium text-foreground hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
          ...
          <button type="button" onClick={() => campaignActions.approveAll(campaign.id)} disabled={pending === 0} className="flex h-10 items-center gap-2 rounded-md bg-primary px-4 text-[15px] font-medium text-primary-foreground hover:bg-primary/85 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:outline-none disabled:opacity-50">
            <Check className="size-4" strokeWidth={2.5} /> Approve all{pending > 0 && <span className="tabular-nums opacity-80">({pending})</span>}
          </button>
          <DropdownMenu>
            <DropdownMenuTrigger render={<button type="button" aria-label="More actions" className="flex size-10 items-center justify-center rounded-md border border-border text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none" />}>
