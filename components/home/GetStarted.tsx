  ...
  return (
    <section className="px-11 pt-[34px]">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={toggle} aria-expanded={!collapsed} className="flex h-9 items-center gap-2 rounded-md border border-border px-4 text-[16px] font-medium text-foreground transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
      ...
      {!collapsed && (
        <>
          <form onSubmit={submit} className="mt-[34px] flex h-[50px] w-[683px] max-w-full items-center gap-4 rounded-lg border border-border bg-card pr-3 pl-5 shadow-[0_1px_2px_rgba(17,24,39,0.06)] focus-within:ring-2 focus-within:ring-primary">
            ...
            <button type="submit" aria-label="Send" disabled={!ask.trim()} className={cn("flex size-8 shrink-0 items-center justify-center rounded-md transition-colors", ask.trim() ? "bg-primary text-primary-foreground" : "bg-primary-soft text-primary/70")}>
          ...
          <div className="mt-16 -mr-11 flex gap-5 overflow-x-auto pr-11 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {quickActions.map((a) => (
              <button key={a.id} type="button" onClick={() => go(a)} className="flex min-h-[128px] w-[315px] shrink-0 flex-col items-start rounded-xl border border-border/70 bg-page px-6 pt-6 pb-6 text-left transition-colors hover:border-foreground/20 hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
