...
- Jeremy's agent is now working on `feat/web-wire-leads` to connect the existing lead table, Origami sourcing state and outreach approval controls to the deployed backend. Other people should take the coach overlay or video lane and avoid editing the leads API adapter and page state until this branch merges.
