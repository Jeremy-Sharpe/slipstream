...
- **Follow-up draft** is written by the same model from the extracted fields, the transcript and the seller profile (`api/app/prompts/follow-up-v1.md`), signed by the rep who owns the next step; a body over 220 words is rejected. The string template is the no-provider fallback. Approve marks it sent and logs an activity; nothing is delivered.
