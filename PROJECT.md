...
- Jeremy's agent is working on `feat/icp-evidence-inventory`: the Intelligence page will read a credential-free live inventory of call and email evidence ready for the next ICP derivation, so a keyless deployment can prove aggregation without pretending it generated a model-backed profile. Other people should record the public demo video and avoid changing the Intelligence status banner until this lane lands.
