...
- Jeremy's agent is now working on `feat/web-wire-emails`: adding labelled email threads to the unified feed and wiring their real provider-neutral ingest, history read, grounded reply draft and approval into the existing conversation format. Other people should avoid the email conversation/API data folders and record the public demo video.
