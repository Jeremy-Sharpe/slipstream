...
- Jeremy's agent completed `feat/coach-release`: pinned, least-privilege GitHub Actions test and package unsigned Linux, macOS and Windows coach installers from a manual run or version tag, with no signing credentials in the repository. Exact main revision `0992496` passed all three hosted runners and uploaded 99 MB Linux, 266 MB macOS and 483 MB Windows artifacts in run `34693377572`; local Linux packaging and the final hostile review are also clean. Other people should record the public demo video.
- Jeremy's agent is working on `feat/ci`: credential-free GitHub checks will protect the web build, API lint/tests, fixture validation/tests and coach tests on every pull request and main push using committed lockfiles. Other people should record the public demo video and avoid adding provider secrets to CI.
