...
- Anna's `live-extraction` lane (PR #5, branch `feat/live-extraction`, 12 September late evening): extraction and the follow-up draft now run on the configured reasoning model for every call, fixture calls included, with a deterministic grounding pass; label copying and the template email are only the no-provider fallbacks. Verified live on gpt-5.4 in memory mode. The extraction eval over all 13 fixtures is in `api/evals/extraction-eval.md`, with gpt-5.4-mini the pick among the OpenAI models and the open-weight comparison credit-limited (the shared OpenRouter key is on a $5 account with about $1.50 left). Contract change for the UI: `ExtractionResult.source` and `DraftResponse.source` say `model`, not `claude`. Production still copies labels until a reasoning key is added to `/etc/slipstream/api.env`.
