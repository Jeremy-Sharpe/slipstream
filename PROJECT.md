...
- Jeremy's agent is working on `feat/database-ci`: an ephemeral Supabase job will apply every migration and run all pgTAP suites without hosted credentials, turning the unapplied email, scorecard and playbook SQL into continuously verified artifacts. Other people should record the public demo video and avoid changing migrations in parallel.
