...
- Jeremy's agent is now working on `feat/web-wire-analysis` to connect the Intelligence/ICP surface to backend results without presenting labelled evaluation data as live. Other people should avoid the Intelligence API adapter and page-level provenance state until that lane merges.
