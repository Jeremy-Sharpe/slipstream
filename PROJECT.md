...
- Jeremy's agent deployed current `main` (`f540be0`) to the VPS and verified exact-revision readiness. The first matching Vercel deploy found that local desktop package artifacts were entering the upload and exceeded its 100 MB file limit; Jeremy's agent is adding an explicit web-only `.vercelignore` before retrying. Other people should keep generated coach installers out of web deployment inputs.
