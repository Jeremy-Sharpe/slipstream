...
- Jeremy's agent completed `feat/web-wire-emails`: two labelled email threads now sit beside calls in the unified feed and execute the deployed provider-neutral ingest, ordered history read, grounded reply draft and exact draft approval. Live API facts and labelled account context remain explicitly separate. The production Wattle Street smoke returned three messages, the correct primary contact, a thread-grounded draft and simulated sent status; lint, build and final hostile review are clean. Other people should record the public demo video.
...
- Jeremy's agent is working on `feat/email-delivery`: approval currently records `status=sent` while explicitly simulating delivery, so this lane will make approval truthful and add a credential-gated, idempotent send path for both call follow-ups and lead outreach. Other people should record the public demo video and avoid changing draft status or delivery code until this lane lands.
