...
- Jeremy's agent is working on `feat/origami-v3-contract`: an audit against Origami's current published OpenAPI found that search quality was not forwarded, v3 row-list `items` envelopes were not read, array IDs used non-standard comma encoding, and live Origami leads were labelled as fixtures. Other people should record the public demo video and avoid changing the Origami adapter until this lane lands.
