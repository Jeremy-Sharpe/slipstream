...
Anna Sekulic reran the Claude models and the production model on 13 September 2026 with prompt `extract-v2`, the optional-`sequence` schema from PR #23, concurrency 1, all thirteen calls including the demo call, every call billed through OpenRouter. The branch was cut before the Eleno rebrand, so this rerun scored the same pre-rebrand Harbourline IT cohort as the 12 September table and is directly comparable to it; the Eleno cohort run is the section after this one. The shared account ran dry after Haiku, the first Sonnet run and six gpt-5.4 calls; it was topped up the same afternoon and gpt-5.4, Opus 5 and a second Sonnet run then completed. Per-model reports are `results/extraction-*-20260913-rerun.md` (the second Sonnet run is `-rerun2`) and the combined table is `results/extraction-comparison-20260913-rerun.md`. Rows with failures are scored over all thirteen calls with a failure counted as a miss on every check, which is what production would see; the completed-call rates follow in prose.
...
## 13 September, Eleno cohort: the two production candidates on the fixtures judges will see

The rebrand replaced five of the thirteen calls outright and edited the rest, so the historical tables above do not describe the fixtures now on `main`. Anna Sekulic ran the two production candidates over the rebranded cohort on 13 September 2026 with `extract-v2`, concurrency 1, all thirteen calls including the demo call, both billed through OpenRouter. Reports are `results/extraction-*-20260913-eleno.md` and the combined table is `results/extraction-comparison-20260913-eleno.md`.
...
**No model clears the predeclared rule on this cohort.** Mistral is five calls behind on deal amount (8 of 13 against 13 of 13) and gpt-5.4 is eight calls behind on objection handling (1 of 13 against 9 of 13), so neither is within one call of the best everywhere and the runner reports no pick. Mistral wins next step by two calls, promises by one and objection handling by eight; gpt-5.4 wins deal amount by five and contact name by one; email, company, headcount and grounding tie. Both parsed every call, and the grounding pass made one repair for Mistral and none for gpt-5.4.

**Decision on the Eleno cohort: keep Mistral, and record deal amount as its known weakness.** With the rule silent, the secondary criteria are the mean judged pass rate (83.8% against 78.9%) and cost ($0.0033 against $0.0304 per extraction, nine times cheaper), and both favour Mistral. Its deal-amount misses are the documented monthly-versus-annual confusion on calls 01, 04, 06, 09 and 11; gpt-5.4's objection-handling score means it almost never labels how an objection was handled the way the labels do. A rep sees a wrong deal value as a wrong number in a field they check, whereas a wrong objection label silently shapes the coaching, so this trade was judged acceptable for the demo and is stated in the README's limitations. The gpt-5.4 cost on this cohort ($0.0304) matches the historical cohort ($0.0298); Mistral's ($0.0033) matches its 12 September figure ($0.0034).
