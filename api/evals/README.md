...
Run repeated comparisons with `UV_OFFLINE=1 uv run python evals/run_scorecard_eval.py --judge openrouter --models default --repeats 3 --launch-budget-usd 5`. This is deliberately a launch gate rather than a hard spend cap: already-running concurrent batches can finish after the threshold is crossed.
