...
def evaluate_case(case: EvalCase, judge: Judge) -> dict[str, Any]:
    ...
    try:
        ...
        return _failed_record(case, expected, error)
    return {
...
def summarise(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "unknown_cost_failures": sum(
            1 for record in records if record["judge_failed"] and record["cost_usd"] is None
        ),
        "unknown_cost_calls": sum(1 for record in records if not record["cost_complete"]),
...
def summarise_model_runs(model: str, repeats: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "unknown_cost_failures": sum(
            repeat["summary"]["unknown_cost_failures"] for repeat in repeats
        ),
        "unknown_cost_calls": sum(repeat["summary"]["unknown_cost_calls"] for repeat in repeats),
...
def compare_models(model_summaries: list[dict[str, Any]]) -> dict[str, Any]:
    candidates = [
        ...
        and summary.get("judge_kind") != "heuristic"
        and summary.get("parse_failures", 0) == 0
        and summary.get("unknown_cost_calls", 0) == 0
    ]
    if not candidates:
...
def run_model(
    *,
    ...
    return {
        ...
        "summary": {
            **summarise_model_runs(model, repeat_results),
            "judge_kind": kind,
        },
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Stop launching new batches after completed billed cost reaches this value.",
    )
    ...
    default_model = "claude-haiku-4-5" if judge_kind == "anthropic" else DEFAULT_JUDGE_MODEL
    models = _models(args.models, args.model or default_model)
    ...
    results = _run_models(
        ...
        launch_budget_usd=args.launch_budget_usd,
    ...
    for result in results:
        if result.get("skipped"):
            ...
            continue
        records = [record for repeat in result["repeats"] for record in repeat["records"]]
    ...
    with ThreadPoolExecutor(max_workers=max(1, concurrency)) as executor:
        ...
        while pending:
            ...
            while (
                ...
                and accumulated_cost <= launch_budget_usd
...
def _checks(expected: dict[str, Any], scorecard: Scorecard) -> dict[str, bool]:
    return {
        "discovery_tolerance": abs(expected["discovery_questions"] - scorecard.discovery_questions)
        <= 1,
...
def _failed_record(case: EvalCase, expected: dict[str, Any], error: JudgeError) -> dict[str, Any]:
    return {
        "call_id": case.call_id,
        "outcome": None,
        "expected": expected,
        ...
        "tokens": {"input": error.input_tokens, "output": error.output_tokens},
        "latency_ms": error.latency_ms,
        "model": error.model,
        "cost_usd": error.cost_usd,
        "cost_complete": error.cost_complete,
        "parse_retries": error.parse_retries,
        "judge_failed": True,
        "error": str(error),
    }
...
def _markdown_report(records: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    ...
    for record in records:
        ...
        if record["scorecard"] is None:
            failed_cost = (
                f"${record['cost_usd']:.6f}" if record["cost_usd"] is not None else "unknown"
            )
            lines.append(
                f"| {record['call_id']} | failed | {expected['discovery_questions']}/- | "
                f"{expected['next_step_secured']}/- | {expected['objection_handling']}/- | "
                f"{expected['rep_talk_ratio']:.2f}/- | {record['latency_ms']} | "
                f"{failed_cost} | {record['parse_retries']} |"
            )
            continue
        scorecard = Scorecard.model_validate(record["scorecard"])
        record_cost = f"${record['cost_usd']:.6f}" if record["cost_usd"] is not None else "unknown"
        lines.append(
            ...
            f"{record['latency_ms']} | {record_cost} | "
...
def _summary_line(summary: dict[str, Any]) -> str:
    ...
    parts.append(f"unknown_cost_failures={summary['unknown_cost_failures']}")
    parts.append(f"unknown_cost_calls={summary['unknown_cost_calls']}")
...
def _passes_exit_gate(model_summaries: list[dict[str, Any]], decision: dict[str, Any]) -> bool:
    ...
    return picked.get("judge_kind") != "heuristic" and all(
        picked["checks"][dimension]["mean_pct"] >= 75 for dimension in JUDGED_DIMENSIONS
    )
