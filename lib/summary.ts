...
const STOP = new Set(["the", "a", "an", "and", "or", "to", "of", "in", "on", "for", "we", "our", "is", "are", "it", "that", "this", "with", "before", "by"]);
...
export function summaryTokens(call: CallRecord): { tokens: Token[]; sources: Source[]; followUps: string[] } {
  ...
  const next = call.fields?.next_step.span ?? null;
  const objection = call.scorecard?.spans.objection ?? null;
  ...
  const who = [call.title, call.headcount ? `${call.headcount} staff` : null].filter(Boolean).join(", ");
  const tokens: Token[] = [
    ...words(lead[call.outcome]),
    ...words(`${f}${who ? ` (${who})` : ""} ${call.trigger ? `is dealing with ${call.trigger.toLowerCase()}` : "came in through this conversation"}`, trigger ?? undefined),
...
export function whyTokens(call: CallRecord): { tokens: Token[]; sources: Source[] } {
  ...
  if (s) {
    ...
    parts.push(`${s.discovery} discovery questions before pricing.`);
    if (call.objections?.[0]) parts.push(`The objection was ${s.objection.replace("_", " ")}.`);
    parts.push(`${rep} spoke ${Math.round(s.talkRatio * 100)}% of the time.`);
  } else if (email) {
    parts.push(call.responseTime ? `${rep} replied in ${call.responseTime}.` : `${rep} has not replied yet.`);
    if (call.fields?.next_step.value) parts.push(`Next: ${call.fields.next_step.value}`);
  } else {
    parts.push("This conversation has not been scored yet.");
  }
  const cites = s ? [s.spans.nextStep, s.spans.discovery, s.spans.objection] : [call.fields?.next_step.span ?? null];
