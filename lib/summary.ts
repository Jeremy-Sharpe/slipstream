import type { CallRecord, Turn } from "./types";
...
export function summaryTokens(call: CallRecord): { tokens: Token[]; sources: Source[]; followUps: string[] } {
  ...
  const lead: Record<CallRecord["outcome"], string> = {
  ...
  const sources: Source[] = [...new Set(cited)].map((i) => call.turns[i]).filter(Boolean).map((t) => ({ i: t.i, name: t.name, text: t.text, t: t.t }));
  const followUps = [
...
export function whyTokens(call: CallRecord): { tokens: Token[]; sources: Source[] } {
  ...
  const parts: string[] = [];
  parts.push(s.nextStepSecured ? `A dated next step was agreed on the call.` : `No dated next step was agreed, which is the pattern in every stalled call.`);
  parts.push(`${s.discovery} discovery questions before pricing${s.discovery >= 5 ? ", above the winning average" : ", below the winning average of 5.6"}.`);
  if (call.objections[0]) parts.push(`The objection was ${s.objection.replace("_", " ")}.`);
  parts.push(`${call.rep.split(" ")[0]} spoke ${Math.round(s.talkRatio * 100)}% of the time.`);
